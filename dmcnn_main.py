...
from forward_backward_pass import forward_backward_pass_dmcnn
import pickle
...
def main():
    ...
    # Optimizer
    optimizer = Adam(dmcnn_network.parameters(), lr=1e-3)
    ...
    for epoch in range(epochs):
        # Train
        dmcnn_network.train()
        dmcnn_network, train_loss, train_mean_psnr, _, _, _, _ = forward_backward_pass_dmcnn(
            dmcnn_network=dmcnn_network,
        ...
        # Validation
        dmcnn_network.eval()
        _, val_loss, val_mean_psnr, _, _, _, _ = forward_backward_pass_dmcnn(
            dmcnn_network=dmcnn_network,
        ...
        if val_loss < lowest_val_loss:
            ...
            best_epoch = epoch
            torch.save(dmcnn_network.state_dict(), Path(best_early_stopping_model_path, f"#{job_idx}_dmcnn_best_model.pt"))
        elif epoch - best_epoch > patience:
            print(f"Early stopping at epoch {epoch}, with lowest validation loss of {lowest_val_loss:.4f}")
            break
    
    # Test
    # Load the best model
    dmcnn_network.load_state_dict(torch.load(Path(best_early_stopping_model_path, f"#{job_idx}_dmcnn_best_model.pt")))
    print(f"Loaded the best model from {Path(best_early_stopping_model_path, f'#{job_idx}_dmcnn_best_model.pt')}")
    dmcnn_network.eval()
    _, test_loss, test_mean_psnr, test_mean_channels_psnr, test_img_targets, test_img_demosaiced, test_img_mosaic = forward_backward_pass_dmcnn(
        dmcnn_network=dmcnn_network,
    ...
    with open(f"narvi_pickles/#{job_idx}_dmcnn_results.pickle", "wb") as f:
        print('\n\n', f"Saving results to narvi_pickles/#{job_idx}_dmcnn_results.pickle")
